...
> ### ⚠ 먼저 `IG0`를 읽을 것
>
> 이 문서는 지표 $F_a(y)(x^a-y^a)$와 두 슬롯 $\partial_i,\partial'_j$로 시작하는데, **그 표기가 계산보다 어렵다.** 변수를 하나로 줄이면 지표도 합도 사라지고 남는 것은 곱의 미분 한 번뿐이며, 그 상태에서 §3.1이 통째로 끝난다 — 게다가 **동전 하나**로 로짓·Fisher 정보·KL이 전부 나온다.
> 순서: **`IG0_한_변수에서_지표_없이.md` → 이 문서.** 아래 지표 계산은 IG0 §3–§5의 한 변수 계산에 합과 $\delta^a_i$만 붙인 것이다.
