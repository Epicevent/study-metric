const SITE_NAV_ORDER = [
  ...
  {"file": "Gr24_핵심계산_주제별완전판.html", "title": "Gr(2,4) 핵심계산 — 주제별 완전판"},
  ...
  {"file": "Grassmann_bundle의_수직접다발과_canonical_class.html", "title": "Grassmann bundle의 수직접다발과 canonical class"},
  ...
  {"file": "Schur_complement와_수평수직_block_positivity.html", "title": "Schur complement와 수평–수직 block positivity"},
...
function rewriteSiteNavigation() {
  ...
  let top = document.querySelector(".sitenav");
  if (!top) {
    top = document.createElement("nav");
    top.className = "sitenav";
    ...
    title.textContent = SITE_NAV_ORDER[index].title;
    top.append(home, title);
    document.body.prepend(top);
  }

  let pn = top.querySelector(".pn");
  if (!pn) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // before KaTeX sees it. Recover only that synthetic malformed element.
