...
DOCS = [
    ...
    dict(track="kahler", kind="md", src="계수합0에서_line_bundle까지.md",
         out="계수합0에서_line_bundle까지.html",
         series="study metric — divisor에서 line bundle까지",
         card=("1+2−2−1=0에서 line bundle까지",
               "한 유리함수의 영점·극을 order → ∮df/f의 winding → ∂∂̄log|f|²의 점 질량으로 세 번 계산한다. 같은 국소식 z가 자명한 선다발에서는 [0]−[∞], O(1)의 단면에서는 [0]이 되는 장면을 대조하고, transition과 Fubini–Study metric을 붙여 δ₀−ωFS/2π=∂∂̄log∥s∥²의 1−1=0까지 직접 확인한다. 검산 19/19")),
    dict(track="kahler", kind="md", src="H1_S2_쌩좌표계산.md",
         out="H1_S2_쌩좌표계산.html",
         series="study metric — de Rham cohomology 쌩좌표 계산",
         card=("구면의 closed 1-form을 쌩좌표로 적분하기",
               "<b>Mayer–Vietoris와 PoU 없이 H¹(S²)=0.</b> 임의의 α=Pdx+Qdy를 두 stereographic 평면에서 꺾은 경로로 직접 적분한다. overlap에서 두 원시함수의 차이를 좌표변환 네 편미분에 넣어 d(F₀−F∞)=0을 확인하고, overlap이 연결되어 상수 하나만 남으므로 전역 원시함수로 붙인다. height 예시와 dθ 실패 후보, S¹의 두 overlap 조각을 대조한다. 검산 34/34")),
