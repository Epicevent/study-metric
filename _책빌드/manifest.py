...
DOCS = [
    ...
    dict(track="dict", kind="md", src="Reeb_CP1에서_Gr24로.md", out="Reeb_CP1에서_Gr24로.html",
         series="study metric — 사전 후속",
         card=("Reeb 벡터장 — S³에서 Σ⁹로, CP¹에서 Gr(2,4)로 (+§10 곡률)",
               "CP¹에서 올린 속도로부터 계량을 직접 만든 뒤 Gr(2,4)의 여섯 성분에 같은 계산을 적용한다. §5에서는 기저쌍 Q → 단위 Plücker 벡터 p=v∧w → 평면 [p]의 세 층을 먼저 구별하고, 기저회전·Plücker 위상·실제 평면변화를 곡선으로 비교한 뒤에야 SU(2)와 S¹이라는 이름을 붙인다. <b>§10</b>: U(2) 접속 Θ의 곡률 𝓕=dΘ+Θ∧Θ=η*∧η (곱미분 두 줄) — 자취가 2i·ω라 <b>머리의 정규화 박스 ω=½Ω(=지뢰 dα=2dA_FS)가 곡률에서 유도</b>되고, g와 ω는 수평 페어링 h_Z의 실부/허부이며, 자취 밖에 k=1엔 없던 su(2) 곡률(𝓕|₀=D*E−E*D)이 선다. 검산 48/48 + 11/11")),
    ...
    dict(track="kahler", kind="md", src="라플라시안에서_RR의_+1로.md", out="라플라시안에서_RR의_+1로.html",
         series="study metric — Gauss–Bonnet 후속",
         card=("라플라시안에서 RR의 +1로 — 무한대의 −2log|z|가 기억한 것",
               "K dA=−Δu dxdy 한 줄에서 출발: u∼−2log|z|의 경계 flux 4π → dz/dw=−z² → TℂP¹≅O(2) → ∫c₁(T)=2 → Todd의 ½c₁이 RR의 +1. 평탄 토러스에서는 전 사슬이 함께 0. 검산 12/12")),
    dict(track="kahler", kind="md", src="Winding_Form에서_Degree까지_한노트.md",
         out="Winding_Form에서_Degree까지_한노트.html",
         series="study metric — winding에서 degree까지",
         card=("원을 세던 적분이 구면의 degree가 되기까지",
               "<b>two-band 논문의 새 선형 입구.</b> 원의 winding form을 ellipse에 직접 넣어 sign det를 얻고, 구면 potential α의 dα=ω<sub>FS</sub>와 Stokes로 local winding 합 = signed preimage 합 = ∫F*ω/2π = degree를 증명한다. 실제 q(20°)의 세 원상에서 2π+2π−2π, q(35°)의 한 원상에서 2π를 계산하여 1↔3 fold가 degree 1을 보존하는 모습을 확인한다. 마지막에 signed degree/Chern 계산과 unsigned singular-curvature 계산을 분리하고 Weierstrass branch를 대조한다. 검산: verify_winding_degree_note.py")),
    dict(track="kahler", kind="md", src="First_Chern과_Ricci_CP1에서_fold까지.md",
         out="First_Chern과_Ricci_CP1에서_fold까지.html",
         series="study metric — First Chern과 Ricci",
         card=("First Chern class와 Ricci curvature — CP¹에서 fold까지",
               "<b>지도교수 피드백을 실제 계산으로 받는 노트.</b> ℂP¹ 한 차트에서 ω<sub>FS</sub>와 ρ를 각각 두 번 미분하고 적분하여 c₁(O(1))=1, c₁(TℂP¹)=2와 ρ=2ω<sub>FS</sub>를 확인한다. 이어 논문의 two-band projector를 직접 미분하여 dĀ=f*ω<sub>FS</sub>, KdĀ=f*ρ를 얻고, (0,0)과 (π,π)에서 Jacobian 부호가 뒤집히는 모습을 본다. 마지막에는 g+εh regularization이 signed Ricci pullback을 복구하지 못하는 이유를 fold normal form으로 검문한다. 검산: verify_chern_ricci_fold.py")),
    dict(track="kahler", kind="md", src="TwoBand_Fold_Sheet_하루손계산.md", out="TwoBand_Fold_Sheet_하루손계산.html",
         series="study metric — two-band 곡률 손계산",
         card=("구면의 양의 곡률면적이 토러스에서 뒤집히는 순간",
               "winding form 적분 하나를 −∮*du → ∫K dA → ∫f*ω<sub>FS</sub>로 옮겨 읽은 뒤, 실제 two-band map의 대각선에서 Jacobian 부호가 +→0→−로 바뀌는 장면을 계산한다. 이어 regular value의 모든 원상을 sheet ID와 함께 추적하여 fold의 1↔3 생성·소멸, cusp의 provenance 교환, signed multiplicity와 degree/Chern 수를 한 계산으로 검산한다.")),
