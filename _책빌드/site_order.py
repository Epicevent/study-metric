...
이 파일은 실제 학습 순서, 이전/다음 순서, 추가 문서 정의만 맡는다.
"""
...
TRACK_OVERRIDE = {
    ...
    '사전은_관찰이_아니다_함수에서_공간으로.html': 'projective',
...
EXTRA_DOCS = [
    ...
    dict(track='relative', kind='existing', out='Quiver_Grassmannian에서_Reineke_Tower까지.html', card=('Quiver Grassmannian에서 Reineke tower까지', '빈 문서 · incidence condition, tautological bundle, acyclic ordering을 tower construction으로 묶는 자리.'), stub=True),
    dict(track='relative', kind='existing', out='Grassmann_bundle의_수직접다발과_canonical_class.html', card=('Grassmann bundle의 수직접다발과 canonical class', '빈 문서 · relative tangent sequence와 Grassmann bundle canonical class formula를 계산할 자리.'), stub=True),
    dict(track='ricci', kind='existing', out='Griffiths_positivity와_Ricci_positivity.html', card=('Griffiths positivity와 Ricci positivity — 무엇이 같고 무엇이 다른가', '빈 문서 · vector-bundle curvature positivity와 Kähler metric의 Ricci positivity를 섞지 않기 위한 비교 자리.'), stub=True),
    dict(track='ricci', kind='existing', out='Twisted_Reineke_Tower_Kahler_metric.html', card=('Twisted Reineke–tower Kähler metric', '빈 문서 · 각 단계의 Plücker curvature와 horizontal–vertical block을 조합하는 metric construction.'), stub=True),
    dict(track='ricci', kind='existing', out='Schur_complement와_수평수직_block_positivity.html', card=('Schur complement와 수평–수직 block positivity', '빈 문서 · tower metric의 block matrix와 큰 twisting parameter에서의 positivity를 계산할 자리.'), stub=True),
    dict(track='ricci', kind='existing', out='Fano_chamber와_Ricci_positivity.html', card=('Fano chamber와 Ricci positivity', '빈 문서 · anticanonical class, twisting parameter, Ricci-positive Kähler metric의 결론을 정리할 자리.'), stub=True),
    dict(track='quantum', kind='existing', out='양자회로_QFIM_Grassmannian_Reineke_응용.html', card=('양자회로 응용 — QFIM·Grassmannian·Reineke tower', '빈 문서 · projective state, QFIM pullback, quiver-constrained circuit를 하나의 응용 지도에 놓는 자리.'), stub=True),
]
