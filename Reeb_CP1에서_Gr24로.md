...
>
> 여기서는 “Boothby–Wang”, “prequantization”을 출발점으로 삼지 않는다. $Gr(2,4)$ 부분에서도 이름을 먼저 쓰지 않는다. 두 벡터가 만드는 평면을 연립일차방정식으로 적고, 그 두 벡터를 $t$에 따라 움직이고, 여섯 개의 $2\times2$ 행렬식을 성분별로 미분한다. §5에서는 같은 평면 안의 기저회전, Plücker 위상만 바꾸는 움직임, 평면 밖으로 나가는 움직임을 먼저 비교한다. 그 뒤에야 앞의 두 움직임을 $SU(2)$와 $S^1$이라 부른다. 일반식은 그 예시에서 이미 일어난 계산을 한 줄로 묶을 때에만 쓴다.
>
...
## §5. 기저가 움직인다는 말과 평면이 움직인다는 말은 다르다

$\mathbb{CP}^1$에서는 복소직선 하나를 단위벡터 $p\in S^3$로 나타냈다. 같은 직선의 다른 단위벡터는

$$p\longmapsto e^{it}p$$

뿐이었다. 그래서 $S^3$에서 보이지만 $\mathbb{CP}^1$에서 보이지 않는 움직임은 위상원 하나였다.

$\mathrm{Gr}(2,4)$에서는 출발부터 한 단계가 더 있다. 한 평면을 적으려면 먼저 그 안의 정규직교 기저 두 개를 골라

$$Q=(v\;w)$$

라고 쓴다. 그러나 평면은 기저쌍 $Q$ 그 자체가 아니다. 같은 평면 안에서도 $v,w$를 섞어 새로운 기저를 만들 수 있기 때문이다. 그다음 두 벡터를 쐐기하여

$$p=v\wedge w$$

를 만들고, 마지막으로 $p$의 공통위상을 지워 평면을 얻는다. 따라서 지금 구별해야 할 것은 세 층이다:

$$
\boxed{
\text{정규직교 기저쌍 }Q=(v\;w)
\ \longrightarrow\
\text{단위 Plücker 벡터 }p=v\wedge w
\ \longrightarrow\
\text{평면 }P=[p]
}.
$$

“$SU(2)$ 방향”, “$S^1$ 방향”이라는 이름부터 붙이면 이 세 층이 섞인다. 먼저 실제 곡선들을 움직여 다음 세 경우를 구별하자.

1. 기저쌍은 움직이지만 $p=v\wedge w$는 움직이지 않는 경우,
2. $p$는 움직이지만 공통위상만 변하여 평면은 움직이지 않는 경우,
3. 평면 자체가 실제로 움직이는 경우.
...
이 속도는 §3의 $R_p=ip$와 정확히 같다. 앞의 세 곡선에서는 기저쌍만 움직이고 $p$가 멈췄다. 네 번째 곡선에서는 $p$가 움직였지만

$$[e^{it}e_1\wedge e_2]=[e_1\wedge e_2]$$

이므로 평면은 여전히 멈춰 있다.

여기에 반드시 비교해야 할 곡선이 하나 더 있다. 첫 번째 기저벡터를 원래 평면 밖의 $e_3$ 방향으로 움직여 보자:

$$v_4(t)=\cos t\,e_1+\sin t\,e_3,\qquad
w_4(t)=e_2.$$

$t\neq0$이면 $v_4(t)$에 $e_3$ 성분이 있으므로

$$P(t)=\operatorname{span}_{\mathbb C}\{v_4(t),e_2\}$$

는 $P_0=\operatorname{span}_{\mathbb C}\{e_1,e_2\}$와 다른 평면이다. 쐐기곱도

$$\begin{aligned}
v_4(t)\wedge w_4(t)
&=\cos t\,e_1\wedge e_2+\sin t\,e_3\wedge e_2\\
&=\cos t\,e_1\wedge e_2-\sin t\,e_2\wedge e_3
\end{aligned}$$

이므로

$$\frac d{dt}\Big|_0(v_4\wedge w_4)
=-e_2\wedge e_3.$$

이 속도는 $p_0=e_1\wedge e_2$의 배수도 아니고 $ip_0$의 배수도 아니다. 새로운 Plücker 성분 $p_{23}$이 생겼다. 따라서 이것은 기저만 바뀐 속도도, 공통위상만 바뀐 속도도 아니고, 아래 평면이 실제로 움직이는 속도다.

이제 세 경우를 계산으로 구별했다:

$$\begin{array}{c|c|c}
\text{위에서 한 일}&p=v\wedge w&[p]\text{가 나타내는 평면}\\ \hline
v,w\text{를 평면 안에서 섞음}&\text{멈춤}&\text{멈춤}\\
v\wedge w\text{의 위상만 바꿈}&\text{움직임}&\text{멈춤}\\
v\text{를 평면 밖으로 움직임}&\text{새 성분으로 움직임}&\text{움직임}
\end{array}
$$

첫째와 둘째는 같은 평면의 기저를 바꾼 움직임이고, 셋째만 $\mathrm{Gr}(2,4)$ 안에서 실제로 남는 움직임이다. 이제 앞의 두 움직임이 왜 각각 세 방향과 한 방향으로 갈리는지 일반적인 기저쌍에서 계산한다.

임의의 평면에 정규직교 기저를 골라

$$Q=(v\;w)\in\mathbb C^{4\times2},\qquad Q^\dagger Q=I_2$$

라 쓰자. 같은 평면의 다른 정규직교 기저를

$$Q'=(v'\;w')$$

라 하자. $v',w'$는 모두 $v,w$의 일차결합이므로 어떤 $2\times2$ 복소행렬 $G$에 대해

$$Q'=QG$$

로 쓸 수 있다. 여기까지는 단지 “같은 평면의 두 기저”라는 말의 행렬 표현이다. 양변에 $Q^\dagger$를 곱하고 $Q^\dagger Q=I_2$를 쓰면 그 행렬은

$$G=Q^\dagger Q'$$

로 정해진다.

이제 $G$가 왜 unitary인지 직접 확인하자. $Q'$의 두 열은 $Q$의 열들이 만드는 평면 안에 있으므로
...
$$\begin{aligned}
G^\dagger G
&=(Q^\dagger Q')^\dagger(Q^\dagger Q')\\
&=Q'^\dagger QQ^\dagger Q'\\
&=Q'^\dagger Q'\\
&=I_2.
\end{aligned}$$

즉 $G\in U(2)$다. 반대로 $G^\dagger G=I_2$이면

$$QG$$

의 두 열은 여전히 정규직교이고, $Q$의 열들을 섞었을 뿐이므로 같은 평면을 만든다. 그러므로

$$\boxed{
\text{같은 평면의 정규직교 기저쌍 전체}
=\{QG:G\in U(2)\}
}.
$$

$U(2)$를 밖에서 가져와 기저 움직임에 억지로 붙인 것이 아니다. 같은 평면의 정규직교 기저를 전부 써 보니 정확히 $QG$이고, 그때 가능한 $G$가 정확히 unitary 행렬인 것이다.
...
여기서 비로소 두 종류의 같은-평면 움직임이 갈린다. $G\in U(2)$이면 $|\det G|=1$이므로

$$\det G=e^{i\theta}$$

꼴이다. 그런데 쐐기곱은 $G$의 네 성분을 따로 기억하지 않고 오직 이 한 수 $\det G$만 기억한다.
...
기저쌍은 움직여도 Plücker 벡터가 전혀 움직이지 않는다. **이 계산을 만족하는 unitary 기저변환들의 모임을**

$$SU(2)=\{G\in U(2):\det G=1\}$$

이라고 부른다. 따라서 “$SU(2)$ 방향이 사라진다”는 말의 내용은 이름이 아니라

$$\boxed{\ \det G=1\quad\Longrightarrow\quad
(QG)_1\wedge(QG)_2=v\wedge w\ }$$
...
이 경우에는 기저가 만드는 평면은 그대로지만 Plücker 벡터가 원을 따라 움직인다. 이 원이 $S^1$이고 그 속도가

$$\frac d{d\theta}\Big|_0e^{i\theta}p=ip=R_p$$

이므로 Reeb 방향이다.

왜 같은 평면의 모든 움직임이 이 두 종류로 나뉘는지도 행렬로 볼 수 있다. $G(t)$가 $I_2$에서 시작하는 $U(2)$ 곡선이면, 작은 $t$에서는 연속인 실수함수 $\theta(t)$를 골라

$$\det G(t)=e^{i\theta(t)}$$

로 쓸 수 있다. 이제

$$H(t):=e^{-i\theta(t)/2}G(t)$$
...
$$\boxed{\ G(t)=e^{i\theta(t)/2}H(t),\qquad H(t)\in SU(2)\ }.$$

같은 평면 안의 임의의 기저 움직임은

1. 쐐기곱에서 완전히 사라지는 $H(t)$의 움직임과
2. 쐐기곱 전체를 $e^{i\theta(t)}$만큼 돌리는 움직임

으로 실제로 분리된다. 그래서 “세 방향과 한 방향”이라고 생각할 수 있는 것이다. 먼저 군의 차원을 외운 뒤 억지로 나눈 것이 아니다.

속도만 놓고도 $3+1$을 직접 볼 수 있다. $G(0)=I_2$인 곡선을 미분하여

$$A:=\dot G(0)$$

라 쓰자. $G(t)^\dagger G(t)=I_2$를 미분하면

$$A^\dagger+A=0.$$

$2\times2$ 행렬의 네 성분에 이 식을 적용하면 대각성분은 순허수이고, 왼쪽 아래 성분은 오른쪽 위 성분의 음의 켤레다. 따라서 실수 $r,s,x,y$를 사용하여
...
첫 번째 행렬에는 실수 $r+s$ 하나만 들어 있다. 두 번째 행렬에는

$$r-s,\qquad x,\qquad y$$

라는 실수 세 개가 들어 있고 trace는 0이다. 따라서 네 개의 같은-평면 속도가

$$\boxed{
\underbrace{\text{trace가 0인 세 속도}}_{p=v\wedge w\text{에서도 사라짐}}
+
\underbrace{\text{공통위상 한 속도}}_{p\mapsto e^{it}p\text{로 남음}}
}
$$

로 갈리는 것을 성분에서 직접 확인했다.

세 층에서 무엇이 사라지는지를 다시 쓰면

$$
\boxed{
Q=(v\;w)
\ \xrightarrow[\det=1\text{인 기저변환이 사라짐}]{\,v\wedge w\,}\
p\in\Sigma^9
\ \xrightarrow[e^{i\theta}p\text{가 사라짐}]{\,[p]\,}\
P\in\mathrm{Gr}(2,4)
}.
$$

첫 번째 화살표에서 사라지는 움직임에 나중에 $SU(2)$라는 이름을 붙이고, 두 번째 화살표에서 사라지는 움직임에 $S^1$ 또는 Reeb 방향이라는 이름을 붙인다. 둘 다 $\mathrm{Gr}(2,4)$ 안의 실제 방향은 아니다. $\mathrm{Gr}(2,4)$ 안의 실제 방향은 앞의 $v_4(t)$처럼 평면 자체를 바꾸는 곡선의 속도다.
